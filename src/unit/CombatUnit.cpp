...
CombatUnit::CombatUnit(int id, char type, int hp,
                       const FieldCoordinates &coordinates, int actionPoints,
                       int cost, int range, int buildTime)
    : Unit(id, type, hp, coordinates), actionPoints(actionPoints), cost(cost),
      range(range), buildTime(buildTime) {}
